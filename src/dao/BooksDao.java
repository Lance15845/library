...
public final class BooksDao {
    ...
    public boolean updateBooks(int choice, Books book){
        ...
        switch (choice){
            ...
            // 更改图书管存量
            case 4:
                String sqlNumExist = "UPDATE BOOKS SET BOOKNUM = ? WHERE BOOKID = ?";
                try {
                    // sql语句预处理
                    pstmt = conn.prepareStatement(sqlNumExist);
                    // 传参
                    pstmt.setDouble(1,book.getBookNum());
                    pstmt.setString(2,book.getBookId());
                    // 执行sql语句
                    int rs = pstmt.executeUpdate();
                    // rs 为更新的条数
            ...
            // 更改图书外借量
            case 5:
                String sqlNumOut = "UPDATE BOOKS SET BOOKNUMBROUGHTOUT = ? WHERE BOOKID = ?";
                try {
                    // sql语句预处理
                    pstmt = conn.prepareStatement(sqlNumOut);
                    // 传参
                    pstmt.setDouble(1,book.getBookNumBroughtOut());
                    pstmt.setString(2,book.getBookId());
                    // 执行sql语句
                    int rs = pstmt.executeUpdate();
                    // rs 为更新的条数
            ...
            // 更改图书借阅次数
            case 6:
                String sqlNumBorrowed = "UPDATE BOOKS SET BOOKNUMBORROWED = ? WHERE BOOKID = ?";
                try {
                    // sql语句预处理
                    pstmt = conn.prepareStatement(sqlNumBorrowed);
                    // 传参
                    pstmt.setDouble(1,book.getBookNumBorrowed());
                    pstmt.setString(2,book.getBookId());
                    // 执行sql语句
                    int rs = pstmt.executeUpdate();
                    // rs 为更新的条数
