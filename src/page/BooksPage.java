...
public final class BooksPage extends ScannerChoice {
    ...
    public static void borrowBooksPage() {
        ...
        ScannerChoice.changedInfoNext("borrowBooksPage");
    }

    /**
     * 7. 图书归还
     */
    public static void returnBooksPage() {
        System.out.println("\t\t 正在执行图书归还操作\n");
        System.out.println("请输入想要归还的书籍名称");
        String bookName = ScannerChoice.scannerInfoString();
        ArrayList<Books> booksList = new QueryPrint().queryBooksKey("", bookName);
        if (booksList.size() <= 0) {
            System.out.println("归还失败，未找到该图书信息");
        } else {
            Books books = booksList.get(0);
            books.setBookNum(books.getBookNum() + 1);
            books.setBookNumBroughtOut(books.getBookNumBroughtOut() - 1);
        ...
        ScannerChoice.changedInfoNext("returnBooksPage");
    }
}
